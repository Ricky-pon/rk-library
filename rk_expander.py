...
ricky_include = re.compile('#include\s*["<](rklib/[a-z_/]*(|.hpp))[">]\s*')

include_guard = re.compile('#.*RK_[A-Z_]*_HPP')
...
def dfs(f: str) -> List[str]:
    ...
    s = open(str(lib_path / 'rk-library' / f)).read()
    result = []
